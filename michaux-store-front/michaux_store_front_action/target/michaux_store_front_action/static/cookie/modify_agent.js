// 代理区域地址存储key modify:agent 省-市-区/县
// 代理区域地址名称存储key modify:agent:name 省-市-区/县
// 管辖区域地址存储key modify:administer:city 市-市-市-市。。。。。。。
// 管辖区域地址存储key modify:administer:area:市： 区-区-区-区。。。。。。。
////////////////////////////////////////////////管辖区域/////////////////////////////////////////////////////////////////////////
...
function get_jurisdiction_city () {
	// 设置title
	var title = cookie.get_cookie("modify:agent:name");
	var aiCode = cookie.get_cookie("modify:agent");
...
function area_ensure () {
	var memorycode = cookie.get_cookie("modify:agent");
	var name = cookie.get_cookie("modify:agent:name");
	...
	cookie.set_cookie_without_time("modify:administer:area:" + memorycode.split("-")[1], areaStr);
	var area = cookie.get_cookie("modify:administer:area:" + memorycode.split("-")[1]);
...
function get_jurisdiction_area (type) {
	// 设置title
	var level = get_level();
	var agentNameStr = cookie.get_cookie("modify:agent:name");
	var agentCodeStr = cookie.get_cookie("modify:agent");
	...
	for(var i=0;i<address_arr.length;i++){
		...
		var available = 0;
		if(level == 1){
			available = address_arr[i].aqId == null ? address_arr[i].aiProvinceAvailable : 1;
		} else if (level == 2) {
			available = address_arr[i].aqId == null ? address_arr[i].aiCityAvailable : 1;
		} else {
			available = address_arr[i].aqId == null ? address_arr[i].aiAreaAvailable : 1;
		}
		if(available == 1){
			if(memory == undefined){
...
function select_all_2 (obj) {
	...
	var code = cookie.get_cookie("modify:agent");
	...
	if(check){
		...
		cookie.delete_cookie("modify:administer:area:" + code.split("-")[1]);
	} else {
		...
		areaStr = areaStr.substring(0, areaStr.length - 1);
		cookie.set_cookie_without_time("modify:administer:area:" + code.split("-")[1], areaStr);
	}
}
// 市级运营中心或者服务站点击区/县
function click_area_2 (obj) {
	// 改变勾选状态,修改记忆的数据 
	var code = cookie.get_cookie("modify:agent");
	var area = cookie.get_cookie("modify:administer:area:" + code.split("-")[1]);
	var check = $($(obj).find("input")).prop("checked");
	// 获取所有勾选的区/县
	var divs = $(".city_jurisdiction_div_context_center").find("div");
	if(check){
...
function clear_memory_address () {
	...
	cookie.delete_cookie("modify:agent");
	cookie.delete_cookie("modify:agent:name");
	// 管辖区域  add:administer:city 和 add:administer:area:市code
	var memoryCityStr = cookie.get_cookie("modify:administer:city");
	if(memoryCityStr != undefined && memoryCityStr != ""){
		var memoryCityArr = memoryCityStr.split("-");
		for(var i=0;i<memoryCityArr.length;i++){
			cookie.delete_cookie("modify:administer:area:" + memoryCityArr[i]);
		}
		cookie.delete_cookie("modify:administer:city");
	}
	var name = cookie.get_cookie("modify:agent:name");
	$("#eiAreaName").val("");
	$("#see").hide();
...
function clear_memory_jurisdiction () {
	// 管辖区域  modify:administer:city 和 modify:administer:area:市code
	var memoryCityStr = cookie.get_cookie("modify:administer:city");
	if(memoryCityStr != undefined && memoryCityStr != ""){
		var memoryCityArr = memoryCityStr.split("-");
		for(var i=0;i<memoryCityArr.length;i++){
...
function get_jurisdiction_city_area (cityCode) {
	...
	for(var i=0;i<address_area_arr.length;i++){
		...
		if(level == 1){
			available = address_area_arr[i].aqId == null ? address_area_arr[i].aiProvinceAvailable : 1;
		} else if (level == 2) {
			available = address_area_arr[i].aqId == null ? address_area_arr[i].aiCityAvailable : 1;
		} else {
			available = address_area_arr[i].aqId == null ? address_area_arr[i].aiAreaAvailable : 1;
		}
		if(available == 1){
			if(memory == undefined){
				html += "<div class='area' code='"+cityCode+"' onclick='click_area(this)'>"+
				"<input type='checkbox' checked=true code='"+address_area_arr[i].aiCode+"' checked=true />"+address_area_arr[i].aiName+"</div>";
			} else {
				if(memory){
					html += "<div class='area' code='"+cityCode+"' onclick='click_area(this)'>"+
					"<input type='checkbox' checked=true code='"+address_area_arr[i].aiCode+"' checked=true />"+address_area_arr[i].aiName+"</div>";
...
function whether_memory (cityCode, type, areaCode) {
	...
	if(type == 1){
		...
		// 判断该区/县是否已选择
		var memoryAreaStr = cookie.get_cookie("modify:administer:area:" + cityCode);
...
function select_all (cityCode) {
	...
	// 当flag为ture时,说明非全部勾选
	var memberCityStr = (cookie.get_cookie("modify:administer:city") == undefined) ? "" : cookie.get_cookie("modify:administer:city");
	if(flag){
		...
		if(checks == undefined || checks == false){
			// 增加
			cookie.set_cookie_without_time("modify:administer:city", memberCityStr + "-" + cityCode);
			// 获取所有的区和县
			var areaStr = "";
			var divs = $(".province_jurisdiction_div_context_right").find("div");
			for(var i=0;i<divs.length;i++){
			...
			areaStr = areaStr.substring(0, areaStr.length - 1);
			cookie.set_cookie_without_time("modify:administer:area:" + cityCode, areaStr);
		} else {
			// 修改对应市的区/县数据
			var areaStr = "";
			for(var i=0;i<divs1.length;i++){
		...
		cookie.set_cookie_without_time("modify:administer:city", newMemberCityStrs);
		cookie.delete_cookie("modify:administer:area:"+ cityCode);
	}
}
...
function click_area (obj) {
	...
	if(check){
		...
		if(total == 1){
			// 增加记忆数据
			var codesss = cookie.get_cookie("modify:administer:city");
			if(codesss == undefined || codesss == ""){
				cookie.set_cookie_without_time("modify:administer:city", cityCode);
			} else {
				cookie.set_cookie_without_time("modify:administer:city", codesss + "-" + cityCode);
			}
			var areaCode = $($(obj).find("input")).attr("code");
			cookie.set_cookie_without_time("modify:administer:area:" + cityCode, areaCode);
		}
	}
	// 记录数据
...
function member_administer_data (obj, type) {
	...
	var cityCode = $(obj).attr("code");
	var check = $($(obj).find("input")).prop("checked");
	var memberCityStr = (cookie.get_cookie("modify:administer:city") == undefined || cookie.get_cookie("modify:administer:city") == "") ? "" : cookie.get_cookie("modify:administer:city") + "-";
	var memberAreaStr = (cookie.get_cookie("modify:administer:area:" + cityCode) == undefined) ? "" : cookie.get_cookie("modify:administer:area:" + cityCode);
	if(type == 1){
		// 点击市,增加或者删除数据
		if(check){
			// 勾选,增加
			...
			cookie.set_cookie_without_time("modify:administer:city", newMemberCityStr);
			cookie.delete_cookie("modify:administer:area:" + cityCode);
		}
	} else {
...
function jurisdiction_ensure () {
	...
	if(memoryCityStr != undefined && memoryCityStr != ""){
		var memoryCityArr = memoryCityStr.split("-");
		for(var i=0;i<memoryCityArr.length;i++){
			var memoryAreaStr = cookie.get_cookie("modify:administer:area:" + memoryCityArr[i]);
			if(memoryAreaStr == undefined || memoryAreaStr == ""){
				// 提示每个市都必须有管辖区域
				$.messager.alert('提示','请重新选择管辖区域','info');
				return;
...
function member_select (aiCode, aiLevel, aiName) {
	var value = cookie.get_cookie("modify:agent"); // 获取已记录的数据
	...
	cookie.set_cookie_without_time("modify:agent:name", name);
	cookie.set_cookie_without_time("modify:agent", value);
}
...
function agent_ensure () {
	var address = cookie.get_cookie("modify:agent"); // 获取已记录的数据
...
//	}
	...
	//console.log(cookie.get_cookie("modify:agent"));
	//console.log(cookie.get_cookie("modify:agent:name"));
}
...
function chakan () {
	...
	if(level == 1){
		...
		if(code.indexOf("-") > 0){
			...
			areaCodeStr = cookie.get_cookie("modify:administer:area:" + cityCodeStr);
			$(".show_select_div_title").text(name.split("-")[1] + "市运营中心");
		} else {
			// 省级运营中心
			cityCodeStr = cookie.get_cookie("modify:administer:city");
			var cityCodeArr = cityCodeStr.split("-");
			for(var i=0;i<cityCodeArr.length;i++){
				areaCodeStr += cookie.get_cookie("modify:administer:area:" + cityCodeArr[i]) + ":";
			}
